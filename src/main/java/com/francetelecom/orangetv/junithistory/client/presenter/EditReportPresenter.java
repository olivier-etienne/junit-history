...
public class EditReportPresenter extends AbstractMainPresenter {
	...
	// ------------------------------- constructor
	public EditReportPresenter(IGwtJUnitHistoryServiceAsync service,
			EventBus eventBus, IEditReportView view) {
		...
		final ClickHandler buttonClickHandler = new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (source != null && source instanceof ButtonViewAction) {
					...
					ViewActionEnum viewActionEnum = ViewActionEnum
							.valueOf(button.getViewAction());
	...
	private void doInitDatas() {
		...
		Integer currentSuiteId = this.params.containsKey(PARAMS_SUITE_ID) ? (Integer) this.params
				.get(PARAMS_SUITE_ID) : IVo.ID_UNDEFINED;
		suiteId = currentSuiteId.intValue();

		final String message = " when editing report!";
		this.rpcService.getEditTestSuiteDatas(suiteId,
				new MyAsyncCallback<VoEditReportDatas>("Error " + message) {

					@Override
					public void onSuccess(VoEditReportDatas datas) {
						mapId2Users = VoIdUtils.getMapId2Item(datas
								.getListUsers());
						view.setActionResult("Success " + message,
								LogStatus.success);
						view.setDatas(datas);
	...
	private void doBeforeUpdateReport() {
		...
		DialogBox dialogBox = WidgetUtils.buildDialogBoxWithOkFocused(
				"Update report ", arguments, null, true, new IActionCallback() {

					@Override
					public void onCancel() {
						view.setActionResult("Update report canceled!",
								LogStatus.warning);
					}

					@Override
					public void onOk() {
						view.setActionResult("Update report in progres...",
								LogStatus.warning);
						doUpdateReport();
					}
	...
	private String[] getUpdateDescription() {
		...
		description[0] = "IPTVKIT : "
				+ ((datas.getIptvkit() == null) ? "undefined" : datas
						.getIptvkit());
		description[1] = "USER    : "
				+ ((user == null) ? "undefined" : user.getName());
		description[2] = "DATE    : "
				+ ((datas.getDate() == null) ? "undefined" : DF.format(datas
						.getDate()));
		return description;
	}
	...
	private void doUpdateReport() {
		...
		final String message = " when updating single report information ("
				+ suiteToUpdate.getName() + ")!";
		this.rpcService.updateTestSuiteInfo(suiteToUpdate,
				new MyAsyncCallback<VoDatasValidation>("") {

					@Override
					public void onSuccess(VoDatasValidation result) {

						if (!result.isValid()) {
							view.setActionResult("Failure " + message,
									LogStatus.warning);
						} else {
							view.setActionResult("Success " + message,
									LogStatus.success);
	...
	private abstract class MyAsyncCallback<T> implements AsyncCallback<T> {
		...
		public void onFailure(Throwable caught) {
			...
			if (this.errorMessage != null) {
				String errorMessage = this.errorMessage + "<br/>"
						+ caught.getMessage();
